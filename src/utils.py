...
def MAP(real, predict):
    # NOTE: previous implementation WRONG!
    # sum = 0.0
    ...
    for idx, val in enumerate(predict):
        if val in set_real:
            num_relevant += 1
            sum += num_relevant / (1.0 + idx)
    return sum / len(set_real)
...
def NDCG(real, predict):
    ...
    for i, predictItem in enumerate(predict):
        if predictItem in set_real:
            itemRelevance = 1
            rank = i + 1
            # dcg += (math.pow(2, itemRelevance) - 1.0) * (math.log(2) / math.log(rank + 1))
            dcg += (math.pow(2, itemRelevance) - 1.0) / math.log(rank + 1, 2)
...
def IDCG(n):
    ...
    for i in range(n):
        ...
        idcg += (math.pow(2, itemRelevance) - 1.0) / math.log(i + 2, 2)
    return idcg